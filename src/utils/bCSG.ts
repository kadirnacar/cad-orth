...
class Vertex {
    ...
    public interpolate(other: Vertex, t: number): Vertex {
        return new Vertex(BABYLON.Vector3.Lerp(this.pos, other.pos, t),
            BABYLON.Vector3.Lerp(this.normal, other.normal, t),
            BABYLON.Vector2.Lerp(this.uv, other.uv, t)
        );
    }
}
...
class Plane {
     ...
     * @param w
     */
    constructor(public normal: BABYLON.Vector3, public w: number) {
    }
     ...
     * @param c Point c
     */
    public static FromPoints(a: BABYLON.Vector3, b: BABYLON.Vector3, c: BABYLON.Vector3): BABYLON.Nullable<Plane> {
        var v0 = c.subtract(a);
        var v1 = b.subtract(a);
 ...
 */
class Node {
    private plane: BABYLON.Nullable<Plane> = null;
    private front: BABYLON.Nullable<Node> = null;
    private back: BABYLON.Nullable<Node> = null;
...
export class CSG {
     ...
     */
    public rotationQuaternion: BABYLON.Nullable<BABYLON.Quaternion>;
    /**
     ...
     */
    public static FromMesh(mesh: BABYLON.Mesh): CSG {
        var vertex: Vertex, normal: BABYLON.Vector3, uv: BABYLON.Vector2, position: BABYLON.Vector3,
            polygon: Polygon,
            polygons = new Array<Polygon>(),
            vertices;
        var matrix: BABYLON.Matrix,
            meshPosition: BABYLON.Vector3,
            meshRotation: BABYLON.Vector3,
            meshRotationQuaternion: BABYLON.Nullable<BABYLON.Quaternion> = null,
            meshScaling: BABYLON.Vector3;
        ...
        var indices = <BABYLON.IndicesArray>mesh.getIndices(),
            positions = <BABYLON.FloatArray>mesh.getVerticesData(BABYLON.VertexBuffer.PositionKind),
            normals = <BABYLON.FloatArray>mesh.getVerticesData(BABYLON.VertexBuffer.NormalKind),
            uvs = <BABYLON.FloatArray>mesh.getVerticesData(BABYLON.VertexBuffer.UVKind);

        if (!uvs) {
            uvs = [positions.length / 3];
     ...
     */
    public buildMeshGeometry(name: string, scene: BABYLON.Scene, keepSubMeshes: boolean): BABYLON.Mesh {
        ...
        if (keepSubMeshes) {
            ...
            mesh.subMeshes = new Array<BABYLON.SubMesh>();

            for (var m in subMesh_dict) {
                materialMaxIndex = -1;
                for (var sm in (<any>subMesh_dict)[m]) {
                    subMesh_obj = (<any>subMesh_dict)[m][sm];
                    BABYLON.SubMesh.CreateFromIndices(subMesh_obj.materialIndex + materialIndexOffset, subMesh_obj.indexStart, subMesh_obj.indexEnd - subMesh_obj.indexStart + 1, <BABYLON.AbstractMesh>mesh);
     ...
     */
    public toMesh(name: string, material: BABYLON.Nullable<BABYLON.Material>, scene: BABYLON.Scene, keepSubMeshes: boolean): BABYLON.Mesh {
