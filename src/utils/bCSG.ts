...
export class ThreeCsg {
    ...
    static toCSG(three_model, offset, rotation) {
        ...
        if (three_model instanceof THREE.BufferGeometry) {
            geometry = new THREE.Geometry().fromBufferGeometry(three_model)
        } else if (three_model instanceof THREE.Mesh) {
        ...
        rotation_matrix = new THREE.Matrix4().makeTranslation(offset.x, offset.y, offset.z);
        rotation_matrix.makeRotationFromEuler(rotation);
        ...
        for (i = 0; i < geometry.faces.length; i++) {
            if (geometry.faces[i] instanceof THREE.Face3) {
                ...
                if (inverted || doubleSided) {
                    var n = geometry.faces[i].normal.clone();
                    n.negate();
    ...
    static getGeometryVertice(geometry, vertice_position) {
        var i;
        ...
        geometry.vertices.push(new THREE.Vertex(vertice_position.x, vertice_position.y, vertice_position.z));
